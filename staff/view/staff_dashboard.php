...
<?php 
$newEvents = count_events_for_each_manager_view();
?>

<?php
echo '<link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">';

// Function to display all events for each manager view
function display_all_events_for_each_manager_view() {
    $myBookings = get_all_events_for_each_manager_view();
    ...
    if ($myBookings !== false) {
        if (!empty($myBookings)) {
            foreach ($myBookings as $book) {
                ...
                echo "<td class='text-left font-medium'>{$student_email}</td>";
                ...
                echo "<td>
                    <button 
                        ...
                        &nbsp;
                    <button
                        class='btn-deny text-white font-bold bg-gray-500 hover:bg-gray-700 px-2 py-1 rounded'
                        data-booking-id='{$booking_id}'
                        data-student-email='{$student_email}'
                        title='Cancel'>
            ...
            echo "<tr><td colspan='4' style='color: green; text-align: center;'><strong>No events found</strong></td></tr>";
        }
    } else {
...
<!-- TOP ROW: Approve Bookings + 3 New Requests -->
    ...
    <div class="flex justify-center">
        <div class="bg-gray-800 text-cyan-400 font-bold px-8 py-4 rounded-md shadow-md">
            Event(s) Queue [<?= $newEvents ?>]
        </div>
    </div>
    ...
    <div class="overflow-x-auto shadow-lg pb-20">
        ...
        <table class="w-full border-collapse">
            ...
            <tbody class="bg-white text-black">
                <?php display_all_events_for_each_manager_view(); ?>
            </tbody>
        </table>
    </div>
...
document.addEventListener('click', function(e) {
    ...
    if (e.target.closest('.btn-approve')) {
        ...
        document.getElementById('confirmTitle').textContent = 'Confirm Completion';
        document.getElementById('confirmMessage').textContent = `Check event by ${currentStudentEmail}? as completed`;
    ...
    if (e.target.closest('.btn-deny')) {
        ...
        document.getElementById('confirmTitle').textContent = 'Confirm Cancellation';
        document.getElementById('confirmMessage').textContent = `Cancel event by ${currentStudentEmail}?`;
        document.getElementById('confirmModal').style.display = 'flex';
    }
});
...
function approveBooking(bookingId, studentEmail) {
    ...
    xhr.open('POST', '../../action/complete_event_action.php', true);
    ...
    xhr.onload = function() {
        ...
        try {
            ...
            if (response.success) {
                document.getElementById('successApproveMessage').textContent = `Event by ${studentEmail} is checked completed ✅`;
...
function denyBooking(bookingId, studentEmail) {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '../../action/cancel_event_action.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    ...
    xhr.onload = function() {
        ...
        try {
            const response = JSON.parse(xhr.responseText);
            if (response.success) {
                document.getElementById('successDenyMessage').textContent = `Event by ${studentEmail} is cancelled ❌`;
